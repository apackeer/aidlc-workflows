...
Text description of the diagram: `next` (passed `$ARGUMENTS` verbatim) returns one directive. The conductor branches on `directive.kind`. For `run-stage`, ordinary `ask`, `invoke-swarm`, and run-then-continue `print` directives it performs the named move and calls `report`, which loops back to `next`. A typed `new-work-routing` ask loops through its declared `next` route instead. For terminal `print`, `error`, and `done` it stops the loop.
