...
A stage that names a scope is `EXECUTE` under it; absence is `SKIP`. The build step `aidlc engine graph compile` *transposes* every stage's `scopes:` list into the compiled EXECUTE/SKIP grid at `<harness-dir>/tools/data/scope-grid.json` — a pure transpose, drift-guarded by `compile --check` exactly like `stage-graph.json`. The grid is what the runtime reads; you never hand-edit it. The 3 initialization stages name every scope (they always run).
...
3. **Recompile.** Run `aidlc engine graph compile` to transpose the tags into `scope-grid.json`, then refresh SKILL.md's compiled scope-table from `aidlc engine gen scope-table`. Run `aidlc engine graph compile --check` and `aidlc engine gen scope-table --check` to confirm no drift (exit 0).
